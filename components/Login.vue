...
const onUserRegistered = () => {
  registrationSuccessMessage.value = "Usuario registrado con éxito!";
  setTimeout(() => {
    registerDialog.value = false; // Cierra el modal después de 3 segundos
    registrationSuccessMessage.value = ""; // Limpiar el mensaje
  }, 3000);
};
...
<template>
  <v-container>
    <v-card>
      <v-img :width="130" :height="130" aspect-ratio="16/9" cover
        src="https://i.ibb.co/d6Q8CCn/be98a687-9e67-492f-9e30-2c54ce96fe0f.jpg"></v-img>
      <v-card-title align-center>Iniciar sesión</v-card-title>
      <v-card-text>
        <v-form @submit.prevent="handleSubmit">
          <v-text-field v-model="user.username" label="Usuario" dense outlined required
            :rules="usernameRules"></v-text-field>
          <v-text-field v-model="user.password" label="Contraseña" dense outlined required :rules="passwordRules"
            :append-inner-icon="flag ? 'fas fa-eye-slash' : 'fas fa-eye'" @click:append-inner="flag = !flag"
            :type="flag ? 'text' : 'password'"></v-text-field>
          <v-card-actions>
            <v-btn block dark type="submit">Ingresar</v-btn>
          </v-card-actions>
          <v-alert v-if="registrationSuccessMessage" type="success">
